...
CA_SECRET_LINE = re.compile(
    r"(?m)^\s*certificate_authority:\s*!secret\s+([A-Za-z0-9_]+)\s*$"
)
...
ALLOWED_REPOSITORIES = frozenset(
    filter(
        None, os.environ.get("ALLOWED_OTA_REPOSITORIES", DEFAULT_REPOSITORY).split(",")
    )
)
MAX_BODY = 128 * 1024
...
def replace_firmware_version(
    text: str, version: str, expected_device_id: str
) -> tuple[str, str]:
    ...
    rendered = INITIAL_OTA_LINE.sub(
        f'    initial_value: "OTA {version} OK"', rendered, count=1
    )
...
def safe_yaml_name(value: Any) -> str:
    if (
        ...
        or YAML_PATTERN.fullmatch(value) is None
...
def redact(value: str, secrets: list[str] | None = None) -> str:
    ...
    text = re.sub(
        r"(?i)(password|passwd|token|secret|api[_-]?key|mqtt_password|wifi_password)(\s*[:=]\s*)\S+",
        r"\1\2<redacted>",
        text,
    )
    text = re.sub(
        r"https?://[^/@\s]+:[^/@\s]+@", "https://<credentials-redacted>@", text
    )
...
def canonicalize_mqtt_ca(yaml_bytes: bytes, secrets_bytes: bytes) -> bytes:
    ...
    try:
        ...
        if not isinstance(secrets, dict):
            raise TypeError("ESPHome secrets file is invalid")
        ...
        secrets[names[0]] = parsed.public_bytes(serialization.Encoding.PEM).decode(
            "ascii"
        )
        ...
        rendered = yaml.dump(
            secrets, Dumper=LiteralDumper, sort_keys=False, allow_unicode=True
        )
...
def save_settings(repository: str, key_id: str) -> None:
    ...
    atomic_private_file(
        SETTINGS_FILE,
        (
            json.dumps({"repository": repository, "key_id": key_id}, indent=2) + "\n"
        ).encode(),
    )
...
class Builder:
    ...
    def list_devices(self) -> list[dict[str, str]]:
        ...
        for path in sorted(CONFIG_DIR.iterdir()):
            if (
                not path.is_file()
                or path.is_symlink()
                or YAML_PATTERN.fullmatch(path.name) is None
                or path.name == "secrets.yaml"
            ):
                continue
            try:
            ...
            if (
                len(versions) == 1
                and len(devices) == 1
                and DEVICE_PATTERN.fullmatch(devices[0])
            ):
                try:
                    suggested = next_patch_version(versions[0])
                except ValueError:
                    suggested = ""
                result.append(
                    {
    ...
    def validate(self, yaml_file: str, device_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if devices != [device_id]:
                raise ValueError("YAML device_id does not match selection")
            output = self._run(
                ["esphome", "config", str(source)], cwd=config_copy, secrets=secrets
            )
    ...
    def create_job(self, yaml_file: str, device_id: str, version: str | None) -> Job:
        ...
        with self.lock:
            if device_id in self.active_devices:
                raise RuntimeError("A build for this device is already active")
    ...
    def _build_boundary(self, job: Job) -> None:
        ...
        try:
            ...
            self._stage(
                job,
                "failed",
                error=redact(str(exc), scalar_secrets(CONFIG_DIR / "secrets.yaml"))[
                    :240
                ],
            )
        finally:
            ...
            with self.lock:
                self.active_devices.discard(job.device_id)
    ...
    def _build(self, job: Job, directory: Path) -> None:
        ...
        secrets = scalar_secrets(CONFIG_DIR / "secrets.yaml")
        config_copy = directory / "config"
        self._copy_config(config_copy, job.yaml_file)
        yaml_path = config_copy / job.yaml_file
        if not yaml_path.is_file() or yaml_path.is_symlink():
            raise ValueError("YAML does not exist")
        repo = directory / "ota"
        self._run(
            ["git", "clone", f"https://github.com/{repository}.git", str(repo)],
            secrets=secrets,
        )
        manifest_path = repo / "manifest.json"
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        current = manifest.get("version")
        if not isinstance(current, str) or SEMVER_PATTERN.fullmatch(current) is None:
            raise ValueError("OTA manifest version is invalid")
        version = job.version or next_patch_version(current)
        if tuple(map(int, version.split("."))) <= tuple(map(int, current.split("."))):
            raise ValueError("New version must be greater than current OTA version")
        job.version = version
        rendered, profile = replace_firmware_version(
            yaml_path.read_text(encoding="utf-8"), version, job.device_id
        )
        yaml_path.write_text(rendered, encoding="utf-8")
        self._append(
            job,
            self._run(
                ["esphome", "config", str(yaml_path)], cwd=config_copy, secrets=secrets
            ),
        )
        ...
        self._append(
            job,
            self._run(
                ["esphome", "compile", str(yaml_path)], cwd=config_copy, secrets=secrets
            ),
        )
        candidates = list(config_copy.rglob("firmware.ota.bin"))
        if len(candidates) != 1:
            raise ValueError("Build did not produce exactly one firmware.ota.bin")
        firmware_name = f"firmware-{version}.ota.bin"
        firmware = repo / firmware_name
        shutil.copy2(candidates[0], firmware)
        self._git_identity(repo)
        self._run(["git", "add", firmware_name], cwd=repo)
        self._run(
            ["git", "commit", "-m", f"Add ESP Anywhere firmware {version}"], cwd=repo
        )
        ...
        parse_and_verify_manifest(
            final_manifest,
            trusted_key_id=settings["key_id"],
            trusted_public_key=signing_public,
            expected_hardware_profile=profile,
        )
        if KEY_FILE.read_bytes() != signing_key_pem:
            raise RuntimeError("Signing key changed during build")
        manifest_path.write_bytes(final_manifest)
        self._run(["git", "add", "manifest.json"], cwd=repo)
        self._run(
            ["git", "commit", "-m", f"Publish signed manifest for {version}"], cwd=repo
        )
        tag = f"firmware-v{version}"
        self._run(["git", "tag", tag], cwd=repo)
        self._stage(job, "publishing")
        if KEY_FILE.read_bytes() != signing_key_pem:
            raise RuntimeError("Signing key changed before publication")
        env = self._git_env()
    ...
    def _copy_config(self, destination: Path, yaml_file: str) -> None:
        if not CONFIG_DIR.is_dir() or CONFIG_DIR.is_symlink():
            raise ValueError("ESPHome config directory is unavailable")
        source_yaml = CONFIG_DIR / yaml_file
        source_secrets = CONFIG_DIR / "secrets.yaml"
        if not source_yaml.is_file() or source_yaml.is_symlink():
            raise ValueError("YAML does not exist")
        if not source_secrets.is_file() or source_secrets.is_symlink():
            raise ValueError("ESPHome secrets file is unavailable")
        ...
        shutil.copytree(
            ...
            ignore=shutil.ignore_patterns(
                ".git", ".esphome", "build", "*.bin", "*.elf", "*.pem", "*.key"
            ),
        )
    ...
    def _git_identity(repo: Path) -> None:
        ...
        Builder._run(
            ["git", "config", "user.email", "esp-anywhere-builder@localhost"], cwd=repo
        )

    @staticmethod
    def _git_env() -> dict[str, str]:
        env = os.environ.copy()
        env.update({"GIT_ASKPASS": str(ASKPASS_FILE), "GIT_TERMINAL_PROMPT": "0"})
        return env
    ...
    def _manifest(
        ...
        document: dict[str, Any] = {
            ...
            "framework": {"name": "esp-idf", "version": "5.5.4"},
            "build_id": f"{version}-{commit[:12]}",
            "git_commit": commit,
            "published_at": datetime.now(timezone.utc)
            .isoformat(timespec="seconds")
            .replace("+00:00", "Z"),
            "release_url": f"https://github.com/{repository}/releases/tag/firmware-v{version}",
            "summary": f"ESP Anywhere firmware {version}",
            "firmware": {
                "url": f"https://raw.githubusercontent.com/{repository}/{commit}/{firmware.name}",
                "size": firmware.stat().st_size,
                "sha256": hashlib.sha256(firmware.read_bytes()).hexdigest(),
            },
            "security": {"key_id": key_id},
        }
        document["security"]["signature"] = base64.b64encode(
            private.sign(rfc8785.dumps(document))
        ).decode("ascii")
        return document
    ...
    def _verify(document: dict[str, Any]) -> None:
        ...
        signature = base64.b64decode(
            unsigned["security"].pop("signature"), validate=True
        )
        ...
        Ed25519PublicKey.from_public_bytes(public).verify(
            signature, rfc8785.dumps(unsigned)
        )

    @staticmethod
    def _create_release(
        repository: str, tag: str, version: str, firmware_name: str
    ) -> None:
        token = TOKEN_FILE.read_text(encoding="utf-8").strip()
        payload = json.dumps(
            {
                "tag_name": tag,
                "name": f"ESP Anywhere firmware {version}",
                "body": f"Signed OTA firmware `{firmware_name}` for ESP Anywhere {version}.",
                "draft": False,
                "prerelease": False,
            }
        ).encode()
        ...
        try:
            ...
            raise RuntimeError(
                f"GitHub release creation failed with HTTP {exc.code}"
            ) from None
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _trusted(self) -> bool:
        return self.client_address[0] in {"127.0.0.1", "172.30.32.2"} or (
            os.environ.get("ALLOW_TEST_CLIENTS") == "1"
            and self.client_address[0] in {"172.17.0.1", "172.18.0.1"}
        )

    def do_GET(self) -> None:
        if not self._trusted():
            self._json(HTTPStatus.FORBIDDEN, {"error": "ingress_only"})
            return
        path = urlparse(self.path).path.rstrip("/") or "/"
        if path == "/healthz":
            self._json(HTTPStatus.OK, {"status": "ok"})
        elif path == "/":
            self._body(HTTPStatus.OK, "text/html; charset=utf-8", HTML.encode())
        elif path == "/api/state":
            self._json(
                HTTPStatus.OK,
                {
                    "devices": self.builder.list_devices(),
                    "key": public_key_info(),
                    "token_configured": TOKEN_FILE.is_file(),
                    "repository": load_settings()["repository"],
                },
            )
        elif match := re.fullmatch(r"/api/jobs/([0-9a-f-]{36})", path):
            job = self.builder.jobs.get(match.group(1))
            self._json(
                HTTPStatus.OK if job else HTTPStatus.NOT_FOUND,
                asdict(job) if job else {"error": "unknown_job"},
            )
        else:
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if path == "/api/validate":
                self._json(
                    HTTPStatus.OK,
                    self.builder.validate(raw.get("yaml_file"), raw.get("device_id")),
                )
            elif path == "/api/build":
                job = self.builder.create_job(
                    raw.get("yaml_file"),
                    raw.get("device_id"),
                    raw.get("version") or None,
                )
                self._json(HTTPStatus.ACCEPTED, asdict(job))
                ...
                atomic_private_file(TOKEN_FILE, token.encode())
                self._json(HTTPStatus.OK, {"stored": True})
            elif path == "/api/settings/key/import":
                with self.builder.lock:
                    if self.builder.active_devices:
                        raise RuntimeError(
                            "Cannot change signing key during an active build"
                        )
                ...
                if (
                    ...
                    raise ValueError("Invalid key import")
                ...
                settings = load_settings()
                save_settings(settings["repository"], key_id)
                self._json(HTTPStatus.OK, public_key_info())
            elif path == "/api/settings/repository":
                settings = load_settings()
                save_settings(raw.get("repository"), settings["key_id"])
                self._json(HTTPStatus.OK, {"repository": load_settings()["repository"]})
            else:
                self._json(HTTPStatus.NOT_FOUND, {"error": "not_found"})
        except RuntimeError as exc:
            self._json(HTTPStatus.CONFLICT, {"error": redact(str(exc))[:240]})
        except Exception as exc:  # noqa: BLE001
            self._json(HTTPStatus.BAD_REQUEST, {"error": redact(str(exc))[:240]})

    def do_DELETE(self) -> None:
        if not self._trusted():
            self._json(HTTPStatus.FORBIDDEN, {"error": "ingress_only"})
            return
        if urlparse(self.path).path != "/api/settings/token":
            self._json(HTTPStatus.NOT_FOUND, {"error": "not_found"})
            return
    ...
    def log_message(self, fmt: str, *args: Any) -> None:
        print(f"builder-web {self.client_address[0]} {redact(fmt % args)}")
